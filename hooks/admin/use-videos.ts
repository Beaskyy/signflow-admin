...
export function useLibraryVideos(filters: LibraryVideoFilters = {}) {
  return useQuery<PaginatedResponse<LibraryVideo>>({
    ...
    queryFn: async () => {
      ...
      if (filters.is_processed !== undefined) searchParams.append("is_processed", filters.is_processed.toString());
      ...
      const response = await apiClient<APIResponse<PaginatedResponse<LibraryVideo>>>(endpoint);
...
export function useLibraryVideo(id: string) {
  return useQuery<LibraryVideoDetail>({
    ...
    queryFn: async () => {
      const response = await apiClient<APIResponse<LibraryVideoDetail>>(`/translation/admin/library-videos/${id}/`);
      return response.data;
    },
    enabled: !!id,
  });
